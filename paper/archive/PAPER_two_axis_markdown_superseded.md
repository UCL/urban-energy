...
Prior work measures energy or access, rarely together; uses inconsistent functional units; relies on modelled rather than metered energy; and works at coarse spatial units (§2). This paper measures both axes for 178,353 of England's 178,605 Output Areas, using metered household and car-travel energy against network-based access with the rate between them, modelling energy per dwelling with household size and floor area as free controls, which resolves the functional-unit ambiguity. The contribution is two measured axes and an explicit rate at national fine-grained scale. The closest antecedent to the rate is the mobility energy productivity metric (Hou et al., 2019), a composite planning score that weights opportunity accessibility by modelled mode energy intensities for United States locations; a measured access-per-kilowatt-hour contrast between dwelling forms, built from metered energy at national scale, has to the authors' knowledge no precedent. The estimand is place-level, and its exposure to residential self-selection is bounded directly.

![Figure 1. The two axes cross: a detached neighbourhood spends about 2.1× a flat's energy per dwelling yet reaches about 1/27 of the amenities on foot (compositional pure-type predictions).](paper/figures/fig1_inversion.png)
...
| Amenities | FSA, GIAS, NHS ODS, OS Open Greenspace | Reachable everyday destinations |
...
Access counts are non-negative and frequently zero, so they are fitted with a Poisson log-link, not a linear model. The access regression holds the income domain but not density (the mechanism under study) and not the overall IMD, whose barriers and living-environment sub-domains are themselves access measures. Households enter as analytic weights, so the effective sample is the number of areas, not the summed household count; with analytic weights the estimator is quasi-Poisson (point estimates with a robust sandwich covariance, no likelihood interpretation), which is what the count contrast requires. Two reading notes on the log models: the energy ratios are ratios of conditional geometric means, consistent with the median-based tables but not restatable as arithmetic-mean ratios, and no retransformation smearing is applied because only ratios, never levels, are reported.
...
A detached neighbourhood spends 2.12× (95% CI 2.01–2.24) a flat's energy per dwelling; the gradient is steepest in travel, 3.07× (2.81–3.35) (Table 2).
...
![Figure 2. Household energy by dwelling type (compositional pure-type predictions): heat plus car travel rises flat to detached, a 2.1× gap.](paper/figures/fig3_energy_gradient.png)

The type columns are observed medians; the ratio is the compositional flat-to-detached estimate (§3.6), not their quotient (Figure 2 plots the pure-type predictions, so its totals differ from the medians in Table 2). The total is the per-type median of household total energy, and need not equal the sum of the separate heat and car medians, because a median is not additive. The heat gap decomposes from 1.60× (1.45–1.76, total) to 1.27× (1.16–1.40) holding household size to 1.17× (1.06–1.30) also holding floor area: on the log scale, size and occupancy account for about 66% (54–85%) of the gap, leaving a direct fabric and exposure effect of 1.17× (a 17% premium). This decomposition is a covariate adjustment on cross-sectional data, not an identified causal mediation, since floor area and household size are themselves selected. An unweighted intercept-OLS specification gives a parallel ladder (1.58× to 1.30× to 1.20×, 60% accounted for), so the exact split is specification-dependent; the compositional figures are primary. The floor-area elasticity of heat is 0.54 and the household-size elasticity 0.5, both below one, so per-square-metre intensity falls with dwelling size and per-person energy falls with occupancy, and neither is used as the unit (§3.6).

![Figure 3. The heat gap decomposed: 1.60× as-is falls to 1.27× at equal family size and 1.17× at equal floor area, so about a sixth of the gap is the building's form.](paper/figures/fig4_decomposition.png)

### 4.2 The access axis

A flat neighbourhood reaches far more at every distance, the gap narrowing as distance grows (Table 3). On foot it reaches 27.2× (95% CI 19–38) the amenities, 52.4× (30–91) the jobs and 12.5× (9–17) the people of a detached one. These on-foot ratios are modelled pure-type contrasts read at 100%-of-type compositions; the corresponding dominant-type median ratio is smaller (amenities 9.7×, jobs 11.6×), and the support-restricted median comparing areas at least half of one type is 12.2×, because few real areas approach a pure vertex and several detached medians are zero. The pure-type contrast states the gap between idealised extremes; the median states the gap between typical flat and detached areas, and both are reported. At each area's own car catchment the raw counts nearly converge (amenities 1.26×, confidence interval 1.05–1.51); at a 25 km drive the flat still leads 11–14×.
...
![Figure 4. On the doorstep, within a 1.6 km walk, a flat neighbourhood reaches 93 food outlets, 48 food shops, 14 schools and 5 GP surgeries; a detached reaches 6, 3, 2 and 0.](paper/figures/fig5_doorstep.png)

![Figure 5. Amenities reachable against network distance by dwelling type (compositional pure-type predictions): a flat reaches about 27× a detached on foot, about 11× at a 25 km drive.](paper/figures/fig6_access_curve.png)

The pattern is national and structural. Across England, dispersed high-energy form covers most of the land with the compact, low-energy cities as pale islands; inside one city the same inversion shows over a few kilometres, the compact core spending less and reaching more (Figures 6 and 7).

![Figure 6. Every English Output Area by energy spent against amenities reachable on foot: the median falls as energy rises.](paper/figures/fig2_country.png)

![Figure 7. England, every Output Area by energy spent per dwelling: high-energy dispersed form covers most of the land, with the low-energy compact cities as pale areas.](paper/figures/fig9_england.png)

![Figure 8. Sheffield mapped twice, energy spent beside access on foot: the high-energy ring is the low-access edge.](paper/figures/fig10_city.png)

### 4.3 The rate

![Figure 9. Amenities reachable per kWh of car travel by dwelling type: a flat returns about 3.9× a detached home (access advantage 1.26× × energy saving 3.1×).](paper/figures/fig7_rate.png)

A flat returns about 3.9× (95% CI 3.1–4.8) the access per kilowatt-hour of car energy, the product of the access advantage and the energy saving. A flat and a detached home reach a comparable count at their own catchments (1.26×, Table 3), but the detached spends about three times the car energy to do so (3.07×, Table 2), so per kilowatt-hour the flat leads 1.26 × 3.07 ≈ 3.9×. The figure depends on how the two axes are conditioned. Matching the access side's income-only conditioning on the energy side raises the energy saving to 4.24× and the rate to about 5.3× (95% CI 4.2–6.7); pricing access instead against an already-electrified travel cost, which removes the dependence between driving distance and catchment (§3.5), lowers the rate to about 2.3× (95% CI 2.0–2.7). The dominant-type median rate is 3.0×. The headline 3.9× sits within this range rather than at its edge.

### 4.4 Decarbonisation scenarios and lock-in

No technology scenario closes much of the energy gap, and none moves the access gap at all (Table 4; all ratios as-lived, per dwelling, compositional). The three levers act differently. Insulation closes the gap: fabric alone, fully deployed, moves it from 2.12× to 1.83×, 20% of the log-gap, because detached homes carry more fabric headroom. Heat pumps do not: fully deployed they move the gap to 2.16×, marginally wider, because a heat pump sharply cuts the low-gap heat component in delivered-energy terms and unmasks the higher-gap car travel. Electrification closes the most of any single lever, to 1.85× (18%), attacking travel where the gap is largest. The Balanced Pathway for 2040 leaves the gap at 1.89×, closing 16%; even full deployment of all three leaves 1.68×, 31% closed and 69% surviving. Fabric plus full electrification without heat pumps gives 1.51×, the conventional lower bound. On foot the access gap is 27.2× in every scenario, unchanged.
...
![Figure 10. The flat-to-detached total energy gap under each decarbonisation lever, beside the on-foot access gap, which is 27× in every scenario.](paper/figures/fig8_scenarios.png)

### 4.5 Robustness

![Figure 11. Every headline ratio with its 95% confidence interval on one log ruler: energy gaps (Detached:Flat) and access gaps (Flat:Detached), clustered by local authority.](paper/figures/fig11_forest.png)

The energy gaps could reflect who chooses detached rather than the form itself. Three checks (§3.8) bound this. Access depends on location and is immune by construction. Adding occupational class (NS-SeC) to the confounds leaves the gaps unchanged (Table 5). An Oster bound on the detached-share gradient gives δ\* ≈ 1.1 for total energy, so unobserved selection would have to be slightly stronger than all observed confounds combined to nullify it; the corresponding δ = 1 bias-adjusted gap is 1.03×, so the bound should be read as survival at the margin, not immunity. For heat δ\* ≈ 0.3, its contrast largely deprivation and tenure. The bound uses the coefficient-based approximation of Oster's estimator on a continuous detached-share vehicle (an intercept OLS with centred R²), not the headline compositional model. The argument therefore rests on total energy and access, not on heat alone.

| Detached-share gradient | raw | + confounds | + NS-SeC | Oster δ\* | adj. at δ=1 |
| --- | --: | --: | --: | --: | --: |
| Total energy | 1.87× | 1.28× | 1.28× | 1.1 | 1.03× |
...
The travel estimate has two free allocator parameters (§3.3), and each rural-urban class total is held fixed, so both only redistribute miles within a class. The commute-distance elasticity is second-order: varying it across 0.15, 0.30 and 0.45 moves the travel contrast only from 2.74× to 2.98× (dominant-type median). The ownership elasticity is the larger assumption: the headline allocator spreads a class's miles proportionally with cars per person, whereas survey evidence is that additional household cars add less than proportional mileage, so proportionality if anything over-allocates to high-ownership detached areas. Sweeping the exponent traces the compositional travel contrast from 3.07× (2.81–3.35) at 1.0 through 2.93× (2.72–3.15) at 0.8 and 2.80× (2.62–2.99) at 0.6 to 2.50× (2.35–2.65) at 0, where the ownership signal is removed entirely and only the measured between-class NTS gradient and household size remain. The floor bounds the travel gap from below whatever the within-class allocation: under any ownership assumption a detached neighbourhood spends at least two and a half times a flat's car energy.
