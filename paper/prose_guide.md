...
**F20. Backloaded fragments.** A fronted condition-fragment followed by clipped coordinated clauses ("At equal household size the band is 1.22, and the gap sits outside it"), a fronted participle before the subject ("Applied in full, the three measures cut..."), or a verbless appositive tail (", 1.35 at equal household size"). Subject and verb lead; the condition folds in after the verb ("The band narrows to 1.22 when household sizes are held equal, in which case the gap still sits outside it"); appositives weave with "which is" or "which comes to". Deliberate contrast fronting ("Rather than relying on models, we measure..."), subjunctive inversion, reason-first ordering, and two-part signposts ("On the building side... On the travel side...") are not this fault. Mechanical catch on new text: grep `, \(so\|and\|but\) [^,;:]\{0,28\}\.` and inspect every fronted phrase that precedes its subject.

**F21. Drama tags.** An aphoristic causal punchline attached to a finding and repeated as a refrain ("untouched in every scenario, because no measure moves a destination", "moves nothing closer", "for as long as they stand"). State the mechanism plainly once, where it belongs; elsewhere state only the fact ("is unchanged in every scenario"). The counter-fault is over-flattening: a sentence that marks a genuinely informative result ("Nothing in the construction forces this convergence") is not a drama tag, and flattening it loses the point.
