"""
OA-level basket index for morphology-linked energy cost of access.

OA-level version of basket_index_v1.py. Identical analytical framework
at ~5x finer spatial resolution (~130 households per OA).
"""
...
from proof_of_concept_oa import load_and_aggregate  # noqa: E402
...
OUT_DIR = PROJECT_DIR / "stats" / "figures" / "basket_oa"
DOC_PATH = OUT_DIR / "basket_oa_summary.md"
...
def save_schema_table(schema: pd.DataFrame) -> Path:
    out_path = OUT_DIR / "table_basket_oa_schema.csv"
    cols = [
        "id",
        "label",
        "group",
...
def save_summary_tables(lsoa: pd.DataFrame, schema: pd.DataFrame) -> tuple[Path, Path]:
    ...
    type_path = OUT_DIR / "table_basket_oa_by_type.csv"
    ...
    dep_path = OUT_DIR / "table_basket_oa_by_deprivation.csv"
...
def save_oa_scores(lsoa: pd.DataFrame, schema: pd.DataFrame) -> Path:
    ...
    out_path = OUT_DIR / "oa_basket_scores.csv"
...
def fig1_category_scores_heatmap(lsoa: pd.DataFrame, schema: pd.DataFrame) -> Path:
    ...
    plt.tight_layout()
    out_path = OUT_DIR / "fig_basket_oa_category_scores_heatmap.png"
...
def fig2_basket_and_cost_bars(lsoa: pd.DataFrame) -> Path:
    ...
    plt.tight_layout()
    out_path = OUT_DIR / "fig_basket_oa_by_type.png"
    plt.savefig(out_path, bbox_inches="tight")
    plt.close()
    return out_path
...
def fig3_energy_vs_basket_scatter(lsoa: pd.DataFrame) -> Path:
    ...
    plt.tight_layout()
    out_path = OUT_DIR / "fig_basket_oa_scatter_energy_vs_basket.png"
    plt.savefig(out_path, bbox_inches="tight")
    plt.close()
    return out_path
...
def fig4_deprivation_gradient(lsoa: pd.DataFrame) -> Path | None:
    ...
    plt.tight_layout()
    out_path = OUT_DIR / "fig_basket_oa_deprivation_gradient.png"
    plt.savefig(out_path, bbox_inches="tight")
    plt.close()
    return out_path
...
def write_summary_document(
    ...
    text = (
        ...
        "\n"
        "1. Read the case note (`paper/case_v2.md`)\n"
        "2. Use this appendix to verify methods, thresholds,"
        " and exact summary values\n"
        "3. Use the CSV outputs in `stats/figures/basket_oa/`"
