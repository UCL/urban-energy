...
def load_data() -> pd.DataFrame:
    ...
    if need_join and BUILDINGS_PATH.exists():
        print("  Joining building morphology from OS/LiDAR...")
        buildings = gpd.read_file(BUILDINGS_PATH)
        cols_avail = [c for c in morph_cols if c in buildings.columns]
        buildings_join = buildings[["geometry"] + cols_avail].copy()
        gdf = gpd.sjoin(gdf, buildings_join, how="left", predicate="within")
        if "index_right" in gdf.columns:
            gdf = gdf.drop(columns=["index_right"])
        matched = gdf[cols_avail[0]].notna().sum()
        print(f"    {matched:,}/{len(gdf):,} UPRNs matched to buildings")
    elif not need_join:
        print("  Building morphology already in dataset")
    else:
        print(f"  WARNING: {BUILDINGS_PATH} not found — no building geometry")
